...
public class AVLTreeImpl<T extends Comparable<T>> extends BSTImpl<T> implements
  ...
  protected boolean isFirstCase(BSTNode<T> node) {
    return calculateBalance(node) < -1;
  }

  protected boolean isSecondCase(BSTNode<T> node) {
    return calculateBalance(node) > 1;
  }

  protected boolean isThirdCase(BSTNode<T> node) {
    return calculateBalance(getRight(node)) > 0;
  }

  protected boolean isFourthCase(BSTNode<T> node) {
